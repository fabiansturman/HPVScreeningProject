...
# Define the bivalent vaccine intervention with customized parameters
vx1_params = dict(
    prob=0.6,
    start_year=2015,
    age_range=[9, 10],
    product='bivalent',
    rel_imm=0.1,          # Relative immunity conferred by the vaccine
    max_dur=50,                # Maximum duration of immunity in years
    wane_immunity='exp',       # Type of waning immunity ('exp' for exponential)
    eff_duration=5             # Effective duration of immunity in years
)
if __name__=="__main__":
    vx1 = hpv.routine_vx(**vx1_params)
