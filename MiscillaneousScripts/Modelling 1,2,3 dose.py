...
sophies_followup_vx1_data = pd.DataFrame({'name':'Sophie\'s vaccine that is just bivalent really',
                                    'genotype':['hpv16', 'hpv18'], #only need to specifify the genotypes that matter for the vaccine!
                                    'rel_imm':[1,1] #defining immunity as fixed values in the dataframe
                                    })

sophies_followup_vx1 = hpv.vx(genotype_pars=sophies_followup_vx1_data, #information from our DataFrame (basically an excel clipping) about our vaccination's effectiveness against particular genotypes (only need to specify the relevant genotypes!)
                              imm_boost = 1.3 #our peak immunity is multiplied by this for subsequent vaccinations
                            )
...
sophies_followup_vx2_data = pd.DataFrame({'name':'Sophie\'s vaccine that is just bivalent really',
                                    'genotype':['hpv16', 'hpv18'], #only need to specifify the genotypes that matter for the vaccine!
                                    'rel_imm':[1,1] #defining immunity as fixed values in the dataframe
                                    })

sophies_followup_vx2 = hpv.vx(genotype_pars=sophies_followup_vx2_data, #information from our DataFrame (basically an excel clipping) about our vaccination's effectiveness against particular genotypes (only need to specify the relevant genotypes!)
                              imm_boost = 1.3 #our peak immunity is multiplied by this for subsequent vaccinations
                            )
...
screen      = hpv.routine_screening(start_year=2000, prob=prob, product='via', label='screen') #Define the routing screening
...
vx2 = hpv.routine_vx(prob=prob, start_year=2008, age_range=[9,15], product=sophies_vx2)

    ##Set up my vaccination
vx1 = hpv.routine_vx(prob=prob, start_year=2008, age_range=[9,15], product=sophies_vx1)
vx3 = hpv.routine_vx(prob=prob, start_year=2008, age_range=[9,15], product=fabians_vx)
...
sim2 = hpv.Sim(pars, interventions = [vx2] , label='2-Doses of Gardasil9® HPV Vaccine')
...
sim1 = hpv.Sim(pars, interventions = [vx1] , label='1-Dose of Gardasil9® HPV Vaccine')
sim3 = hpv.Sim(pars, interventions = [vx3] , label='3-Doses of Gardasil9® HPV Vaccine'
               )
...
#msim = hpv.parallel(baseline_sim, sim_vx_only, sim_3)
msim.plot('cancer_incidence')

#msim.plot()

#msim.plot(style='simple', legend_args={'frameon':True}, style_args={'ytick.direction':'in'});
'''
sim1.run()
sim2.run()
sim3.run()
