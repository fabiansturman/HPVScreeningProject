...
screening_algorithm_stub_name = 'V15U10A5' #set the name we will be using for screening algorithms
screening_algorithm = screeningAlgorithms.get_interventions(v=15,u=10,a=5)
"""                                                         ^
...
seeds = [0,1,2] # Define seeds we are generating the results over (these raw seeds will be offset by parameterisation number, so that we get a much wider set of seeds when iterating over different parameters from the calibration )
