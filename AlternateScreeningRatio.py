#Implementing Alternate Screening Pathway (ASP) Ratio (R): For a range of values for lambda, we keep the total number of regular screenings the same but multiply the number of screenings for the unvaccinated population by lambda . 
...
lambdas = [2.5,3.0]#[1.5,2.0]#[0.5,1.0]
...
if __name__=="__main__":
    ...
    for pars in parss:
        ...
        for l in lambdas:
            ...
            run_name = f"results/AlternateScreeningRatio/RatioResults_seed{seeds[seed_number]}_basepars{base_parss_number}_lambda{l}"
            ...
            def routine_screen_eligible(sim):
                ...
                for i in range(sim.n): #sim.n==len(sim.people)
                    if i in sim.people.last_hpv_result.keys():
                        ...
                        relative_screens=l if (not vaccinated) else (1-l+l*v)

                        last_hpv_result = sim.people.last_hpv_result[i]
                        age = sim.people.age[i]
                        date_screened = sim.people.date_screened[i]

                        if last_hpv_result==1 and 24<=age<50 and sim.t>date_screened+3/(sim['dt']*relative_screens):
