#Implementing Alternate Screening Pathway (ASP) F: Normal NHS until 2025. Starting 2025, unvaccinated women ALWAYS have a 3 year callback
...
if __name__=="__main__":
    ...
    for pars in parss:
        ...
        run_name = f"results/AlternateScreeningF/FResults_seed{seeds[seed_number]}_basepars{base_parss_number}"
        ...
        def routine_screen_eligible(sim):
            ...
            for i in range(sim.n): #sim.n==len(sim.people)
                if i in sim.people.last_hpv_result.keys():
                    #unvaccinated after 2025 get shorter (or same) callback intervals - always 3 years
                    callback_intervals = (3,5) if (sim.t < tp_switch or sim.people.vaccinated[i]) else (3,3)
