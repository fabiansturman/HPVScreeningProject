#Implementing Alternate Screening Pathway (ASP) Ratio (S): iterate over pairs (lambda, mu) where lambda is the scale factor for time in between checkups for unvacc, and mu for vacc 
#NOTE: ONLY ADJUSTING TIME BETWEEN REGULAR SCREENS WHEN THE LAST HPV TEST WAS NEGATIVE. IF IT IS A FOLLOWUP SCREEN, OR THE LAST HPV TEST WAS POSITIVE, STILL 3YRS WITH THIS ALG 
...
#Parameters for the screening algorithm modelling itself
tp_switch = int((2025-1970)/0.25) #the correct timepoint at which to switch screening methods, as long as we start simulating at 1970 and have dt=0.25
v=0.8 #proportion of people who are vaccinated once we reach stable state (I think its this? TODO: update vacccination regieme so it is this, as that is pretty much waht it should be!) #TODO: update this with something better, maybe updating v every 5 years or smth to match the proportion of vaccinated people more dynamically
lambdas = [0.5,1,1.5,2,3,5]
mus = [0.5,1,2,5,10]
...
#cartesian product of lambdas and mus
lms = []
for l in lambdas:
    for m in mus:
        lms.append((l,m))

#lms has 30 elements, so run 6 scripts for 5 elements each
#lms = lms[:5]
#lms = lms[5:10]
#lms = lms[10:15]
#lms = lms[15:20]
...
if __name__=="__main__":
    ...
    for pars in parss:
        ...
        for l,m in lms:
            ...
            run_name = f"results/AlternateScreeningS/SResults_seed{seeds[seed_number]}_basepars{base_parss_number}_lambda{l}_mu{m}"
            ...
            def routine_screen_eligible(sim):
                ...
                for i in range(sim.n): #sim.n==len(sim.people)
                    if i in sim.people.last_hpv_result.keys():
                        ...
                        relative_screens=l if (not vaccinated) else m
                        ...
                        if last_hpv_result==-1 and 24<=age and sim.t > date_screened+5/(sim['dt']*relative_screens):
                            for_screening[i] = True
