#Implementing Alternate Screening Pathway (ASP) D2: Normal NHS until 2025. Starting 2025, vaccinated women have a 50year/50year callback (rather than 5 year/3year)
...
if __name__=="__main__":
    ...
    for pars in parss:
        ...
        run_name = f"results/AlternateScreeningD2/D2Results_seed{seeds[seed_number]}_basepars{base_parss_number}"
        ...
        def routine_screen_eligible(sim):
            ...
            for i in range(sim.n): #sim.n==len(sim.people)
                if i in sim.people.last_hpv_result.keys():
                    ...
                    callback_intervals = (3,5) if (sim.t < tp_switch or not sim.people.vaccinated[i]) else (50,50)
