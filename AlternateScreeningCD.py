#Implementing Alternate Screening Pathway (ASP) CD: Normal NHS until 2025. Starting 2025, if a vaccinated woman has a normal cytology, she is returned immediately to regular HPV testing pool (i.e. no consecutive HPV tests), and vaccinated regular intervals changed from 5/3 to 10/10
...
if __name__=="__main__":
    ...
    for pars in parss:
        ...
        run_name = f"results/AlternateScreeningCD/CDResults_seed{seeds[seed_number]}_basepars{base_parss_number}"
        ...
        def update_needs_consec_screening_2(sim, name="first_cytology"):
            ...
            for i in sim.get_intervention(name).outcomes['normal']: #iterate over all negative outcomes from first cytology
                if not sim.people.vaccinated[i] or sim.t<tp_switch: #unvaccinated women always get these followups, vaccinated ones only get this up to end of 2024
